...
export class SeatService implements OnApplicationBootstrap {
  ...
  async createSeatMap(screenId: number, request: CreateSeatMapRequest) {
    return await this.dataSource.transaction(async (manager) => {
      ...
      const showtimeRepository = manager.getRepository(Showtime);
      ...
      const isShowtimeInFutureExisted = await showtimeRepository.exists({
        where: {
          screen: { id: screenId },
      ...
      await seatRepository.softDelete({ screen: { id: screenId } });
